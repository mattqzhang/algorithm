...
public static int maxProfit(int[] prices) {
    ...
    **/
    for(int p:prices){          // Assume we only have 0 money at first
        ...
        lowestBuyPrice2 = Math.min(lowestBuyPrice2, p-maxProfit1);  // The min if we've just buy  2nd stock so far.
        maxProfit1 = Math.max(maxProfit1, p-lowestBuyPrice1);       // The maximum if we've just sold 1nd stock so far.
        lowestBuyPrice1 = Math.min(lowestBuyPrice1, p);             // The maximum if we've just buy  1st stock so far.
    }
